 ...
 */
__global__
void calculate_index_hash(GHashRelContainer* target, tuple_indexed_less cmp) {
    ...
    for (u64 i = index; i < outer_table->tuple_counts; i += stride) {
        ...
        while (true) {
            ...
            bool cmp_res = tuple_eq(inner_table->tuples[position],
                                    outer_tuple,
                                    join_column_counts);
            // if (outer_tuple[0] == 1966 && outer_tuple[1] == 8149  && iter == 1) {
            //     printf("init pos %lld, map_size: %lld, hash: %lld\n",
            //        index_position, inner_table->index_map_size,
            //        hash_val);
            //     printf("%d wwwwwwwwwwwwwwwwwwwwww %lld, %lld outer: %lld, %lld; inner: %lld, %lld;\n",
            //            cmp_res,
...
void load_relation(GHashRelContainer* target, int arity, column_type* data, u64 data_row_size,
    ...
    if (build_index_flag) {
        ...
        calculate_index_hash<<<grid_size, block_size>>>(
            target_device, tuple_indexed_less(target->index_column_size, target->arity));
...
void binary_join(GHashRelContainer* inner, GHashRelContainer* outer,
    ...
    // print_hashes(inner, "inner hashes");
    // checkCuda(cudaDeviceSynchronize());
    timer.start_timer();
    get_join_result_size<<<grid_size, block_size>>>(inner_device, outer_device, outer->index_column_size, result_counts_array, iter);
    checkCuda(cudaDeviceSynchronize());
    ...
    load_relation(output_newt, output_newt->arity, join_res_raw_data, total_result_rows, 1, 0.6,
...
void transitive_closure(GHashRelContainer* edge_2__2_1, GHashRelContainer* path_2__1_2,
    ...
    checkCuda(cudaMalloc((void**) &tuple_full, path_full->tuple_counts*sizeof(tuple_type)));
    cudaMemcpy(tuple_full, path_full->tuples, path_full->tuple_counts*sizeof(tuple_type),
               cudaMemcpyDeviceToDevice);
    ...
    while(true) {
        ...
        checkCuda(cudaMalloc((void**) &tuple_full_buf, (current_full_size+path_delta->tuple_counts)*sizeof(tuple_type)));
        checkCuda(cudaDeviceSynchronize());
        tuple_type* end_tuple_full_buf = thrust::merge(thrust::device,
                tuple_full, tuple_full+current_full_size,
                path_delta->tuples, path_delta->tuples+path_delta->tuple_counts,
                tuple_full_buf,
                tuple_indexed_less(path_delta->index_column_size, path_delta->arity));
        checkCuda(cudaDeviceSynchronize());
        ...
        // std::cout << "iteration " << iteration_counter << " finish dedup new tuples : " << deduplicate_size
    ...
    checkCuda(cudaMalloc((void**) &new_full_raw_data, current_full_size*path_full->arity*sizeof(column_type)));
    flatten_tuples_raw_data<<<grid_size, block_size>>>(tuple_full, new_full_raw_data,
                                                       current_full_size, path_full->arity);
    checkCuda(cudaDeviceSynchronize());
    // cudaFree(tuple_merge_buffer);
    load_relation(path_full, path_full->arity, new_full_raw_data, current_full_size,
                  path_full->index_column_size, path_full->index_map_load_factor,
    ...
    std::cout << "get size time: " << join_get_result_time
              ...
              << " ; rebuild newt time: " << rebuild_newt_time
              << std::endl;;
...
void graph_bench(const char* dataset_path, int block_size, int grid_size) {
    ...
    // binary_join(edge_2__2_1, edge_2__1_2, result_newt, reorder_array, 2, grid_size, block_size, 0);
